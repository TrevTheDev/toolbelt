...
const enhanceChainNodeWithSync = <Chain extends ChainGenerics>(chainNode: ChainNode<Chain>) =>
...
function enhancedChain<T extends ValidAsyncFn[], Options extends PartialOptions = {}>(
  ...
  const chainNodeWrapper = <Chain extends ChainGenerics>(
    ...
    const nextChainNodeFn = addAsyncFns<ChainNode<Chain>, EnhancedChainNode<Chain, FinalOpts>>(
      chainNodeWrapper as any,
      asyncFnEnhancer,
      chainNode,
    )
  ...
  const chainFn = addAsyncFns<any, EnhancedChainFn<FinalOpts>>(
    chainNodeWrapper as any,
    asyncFnEnhancer,
    chain,
  )
