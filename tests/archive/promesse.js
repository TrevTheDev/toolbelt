...
const { default: promise } = require('../../build/cjs/promesse.js')
...
for (const [desc, P] of [
  ...
  describe(desc, () => {
    describe('A chain', () => {
      const p = P.resolved(0).then(add(2)).then(add(3))

      it('can be continued', () => p.then(sub(4)).then((x) => x.should.equal(1)))

      it('can be continued different ways', () => p.then(sub(5)).then((x) => x.should.equal(0)))

      it('can be continued different ways', () => p.then(add(1)).then((x) => x.should.equal(6)))

      describe('rejected values are handled by the right handler', () => {
        const f = P.rejected(1)

        it('the second handler is called', () => f.then(undefined, (x) => x.should.equal(1)))

        it('turns failure into success', () => f.then(undefined, sub(1)).then((x) => x.should.equal(0)))

        it('unless the error handler rejects', () => f.then(undefined, () => P.rejected(2)).then(undefined, (x) => x.should.equal(2)))
      })
    ...
    describe('Promises are async in a weird way', () => {
      ...
      it('the reject handler at the end of a chain is called on reject', () => {
        ...
        return d.promise
          .then(add(2))
          .then(sub(1))
          .then(undefined, (x) => x.should.equal(0))
      })
    })
  })
  ...
  describe('When a handler returns a thenable its state is assumed', () => {
    it('the state of a resolved promise is assumed', () => {
      ...
      return P.resolved('dummy')
        .then(() => P.resolved(y))
        .then((x) => x.should.equal('hey'))
    })

    it('the state of a deep resolved promise is assumed', () => {
      const y = {
        then: (onFulfilled) => onFulfilled(P.resolved('hey')),
      }
      return P.resolved('dummy')
        .then(() => P.resolved(y))
        .then((x) => x.should.equal('hey'))
    ...
    it('the state of a thenable that throws before resolving is assumed', () => {
      ...
      return P.resolved('dummy')
        .then(() => x)
        .then(undefined, (e) => chai.expect(e).to.be.an('error'))
    })
